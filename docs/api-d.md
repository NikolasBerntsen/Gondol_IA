...
2. Si es `RECALL`, se ejecuta `RecallMatchingService.matchAnnouncement(id)` (fundación, §12.4): pone los lotes
   ...
   notifica `RECALL_ALERT` (link `/app/recalls?match={matchId}`, el deep link de la coincidencia) y hace push de
   `RecallAlertMessage` a `/user/queue/security-alerts` de cada usuario con acceso a esa sucursal, y actualiza
...
| Archivo | Qué es |
|---|---|
| `pages/OwnerAnnouncementsPage.tsx` | `/owner/announcements`. Adopción arriba (avisos, clientes alcanzados, destinatarios; "clientes alcanzados" son los comercios **distintos** de los recalls en curso, `recalls.affectedTenantsTotal` de `GET /api/platform/metrics`, no la suma por recall), filtro por tipo, tarjetas con franja de severidad, detalle desplegable con la ficha del recall y el desglose de coincidencias, y archivado con diálogo que explica el efecto. |
...
| `pages/RecallsPage.tsx` | `/app/recalls`. KPI (sin confirmar, confirmados sin retirar, unidades en cuarentena), filtros Para resolver / Resueltos / Todos, una tarjeta por coincidencia con sucursal, lote, vencimiento, cantidad y estado, "Entendido" y el diálogo "Retirar del lote del stock" (resolución + nota); las resueltas muestran quién la confirmó, quién la resolvió y la nota. Con `?match=<id>` ubica la coincidencia en todas las sucursales accesibles (`X-Branch-Id: all`), cambia el selector a su sucursal si hace falta, elige el filtro donde está y la resalta; llegan con ese parámetro tanto el botón del diálogo de seguridad como el link de la notificación `RECALL_ALERT` de la campana. Con una sucursal elegida avisa igual si hay alertas sin resolver en las otras sucursales del usuario, con el botón para verlas (para el que entra desde el menú, sin `?match=`). |
| `components/SecurityAlertHost.tsx` | Montado por `AppShell` para usuarios de comercio. Al montar pide las coincidencias `OPEN` de **todas** las sucursales accesibles (`X-Branch-Id: all`, sin importar la elegida en el topbar), se suscribe a `/user/queue/security-alerts` y muestra un **diálogo rojo bloqueante** (`alertdialog`, sin cierre por fondo ni Esc) con producto, lote, vencimiento, código, sucursal, unidades, motivo y pasos numerados, con sonido y vibración. El encabezado dice cuándo se detectó el lote (`matchedAt`), que puede ser posterior a la publicación del recall. "Entendido" confirma y muestra la siguiente de la cola; "Ver detalle y retirar del stock" navega a `/app/recalls?match=<id>`. Para el jefe y el cajero, que no pueden resolver, el botón es "Ver detalle" y el último paso les pide avisarle al administrador o a un empleado. |
